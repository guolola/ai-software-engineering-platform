...
function Badge({
  ...
  render,
  ...props
}: useRender.ComponentProps<'span'> & Omit<VariantProps<typeof badgeVariants>, 'variant'> & { variant?: VariantProps<typeof badgeVariants>['variant'] | 'success' | 'warning' | 'info' }) {
  // Success keeps its semantic color independently of the selected theme preset.
  const templateVariant = variant === 'warning' || variant === 'info' ? 'secondary' : variant;
  return useRender({
    defaultTagName: 'span',
    props: mergeProps<'span'>(
      {
        className: cn(badgeVariants({ variant: templateVariant }), className)
      },
      props
    ),
    render,
